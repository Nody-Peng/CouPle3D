...
function wardrobeView() {
  ...
  $('modal-content').innerHTML=`<p class="small">選擇後，世界中的角色會立即預覽。儲存後伴侶也會看見新造型。</p><div class="avatar-options">${state.bases.filter(base=>lookFilter==="all"||base.startsWith(lookFilter)).map(base=>`<button data-base="${base}" aria-pressed="${a.base===base}">${labels[base]||((base.startsWith("female")?"女生":"男生")+" · 造型 "+(base.charCodeAt(base.length-1)-96))}</button>`).join('')}</div><div class="row">${['hat','glasses','bag'].map((slot,i)=>`<label>${['帽子','眼鏡','背包'][i]}<select data-slot="${slot}"><option value="none">不配戴</option>${state.catalog.filter(x=>x.type===slot&&owned(x.id)).map(x=>`<option value="${x.id}" ${a[slot]===x.id?'selected':''}>${x.name}</option>`).join('')}</select></label>`).join('')}</div><div class="notice">配件可在小商店購買。所有基本造型皆可自由使用。</div><div class="row"><button class="primary" id="save-avatar">保存造型</button><button id="to-shop">去看看配件</button></div>`;
