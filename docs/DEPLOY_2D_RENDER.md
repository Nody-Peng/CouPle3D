...
手機請橫向遊玩，左側搖桿移動、右側按鈕與附近物件互動；上方「選單」提供聊天、日記、家具、約會、抱抱、默契卡、暱稱與合照。手機與電腦可混合連線，使用同一個 Render 網址與共同存檔。照片在瀏覽器版會下載 PNG。

手機版有橫向提示、瀏海安全邊界與可捲動輸入視窗；開啟選單、切換背景或取消觸控會停止移動。全螢幕鍵僅在瀏覽器支援時顯示。已用 Chromium 手機模擬與實際觸控事件測試；iPhone Safari、Android 真機效能、實際軟體鍵盤仍需部署後在你們的手機驗收，不是原生 App。
...
測試命令：`npm test`、`node tools/verify-home-network.mjs`、`node tools/verify-home-web.cjs`、`node tools/verify-home-mobile.cjs`。原生連線測試需要本機 Godot 與素材；後兩項需要 Playwright 與 Chrome，會啟動兩個隔離登入的真正 WebGL 客戶端。手機測試涵蓋搖桿、觸控取消、互動、聊天、日記、方向切換、縮小的鍵盤視窗與重新登入。實際 Render 網址及你們兩地網路仍需部署後驗收。
